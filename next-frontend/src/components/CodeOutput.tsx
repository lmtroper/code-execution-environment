...
import { runCode, submitCode, fetchSubmissions, formatDateString } from '@/utils';
...
const CodeOutput: React.FC<CodeOutputProps> = ({ code, output, onCodeOutput }) => {
  const [successMessage, setSuccessMessage] = React.useState<string | null>(null); // Success message
  ...
  return (
    <div className="bg-gray-100 border rounded-md">
      ...
      <div className='p-4 flex justify-between items-center'>
        <div>
          <button className='px-9 py-4 rounded-md border border-gray-500  hover:bg-slate-200' onClick={handleFetchSubmissions}>Submissions</button>
            <Modal
        ...
        >
          <div className="h-96 overflow-y-auto flex-col-reverse">
            {submissions.map((submission) => (
              <div key={submission.id} className='mt-2 p-2 border-b border-gray-300 flex items-center justify-between'>
        ...
        <div className='flex justify-end'>
          <button className='bg-slate-300 px-7 py-4 mr-6 rounded-md hover:bg-slate-400' onClick={handleTestCode}>Test Code</button>
          <button className='bg-slate-500 px-9 py-4 rounded-md hover:bg-slate-600' onClick={handleSubmitCode}>Submit</button>
        </div>
      </div>
      {output !== '' &&
